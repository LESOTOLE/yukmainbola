...
import { addVenue } from "@/app/actions/admin-venues";
import { Loader2, Plus, X } from "lucide-react";
import { Button } from "@/components/ui/button";

export default function VenueFormSheet() {
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm transition-opacity">
          ...
          <div className="w-full max-w-md h-full bg-surface border-l border-border shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            <div className="p-6 border-b border-border flex items-center justify-between">
              <h2 className="text-xl font-bold text-text">Tambah Lapangan Baru</h2>
              <button 
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <form id="venue-form" onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2">
                  <label htmlFor="name" className="text-sm font-medium text-text">Nama Lapangan <span className="text-danger">*</span></label>
                  <input 
                    type="text" 
                    id="name" 
                    name="name" 
                ...
                <div className="space-y-2">
                  <label htmlFor="address" className="text-sm font-medium text-text">Alamat Lengkap <span className="text-danger">*</span></label>
                  <textarea 
                    id="address" 
                    name="address" 
                    required 
                    rows={3}
                    placeholder="Contoh: Jl. Pintu Satu Senayan, Jakarta"
                    className="w-full bg-background border border-border rounded-md px-3 py-2 text-text focus:outline-none focus:ring-2 focus:ring-primary/50"
                  ></textarea>
                </div>

                <div className="space-y-2">
                  <label htmlFor="maps_url" className="text-sm font-medium text-text">Link Google Maps (Opsional)</label>
                  <input 
                    type="url" 
                    id="maps_url" 
                    name="maps_url" 
                ...
                <div className="space-y-2">
                  <label htmlFor="image" className="text-sm font-medium text-text">Foto Lapangan</label>
                  <input 
                    type="file" 
                    id="image" 
                    name="image" 
            ...
            <div className="p-6 border-t border-border bg-surface/50">
              <div className="flex gap-3 justify-end">
                ...
                <Button type="submit" form="venue-form" disabled={isLoading}>
